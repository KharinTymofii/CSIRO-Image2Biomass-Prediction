...
class BiomassDINOv3(pl.LightningModule):
    """
    DINOv3 model for biomass prediction with:
    ...
    def __init__(
        ...
        Args:
            ...
            backbone_name: DINOv3 model from Hugging Face
        ...
        self.backbone_hidden_dim = self.backbone.config.hidden_size
        ...
        if self.backbone_hidden_dim > 10_000:
            feature_dim = 4096
    ...
    def forward_features(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Get last hidden state: [B, N, C] where N = num_patches + 1 (CLS token)
        features = outputs.last_hidden_state  # [B, N, C]
        
        # Option 1: Use CLS token (first token)
        # pooled = features[:, 0, :]  # [B, C]
        
        # Option 2: Average all tokens (including CLS)
